...
class FeedbackModal(Modal):
    def __init__(self, ticket_id: str, cog):
        ...
        self.rating = TextInput(
            label="Nota (1-5)",
            placeholder="De 1 a 5, como foi seu atendimento?",
        ...
        self.resolved = TextInput(
            label="Problema resolvido? (sim/não)",
            placeholder="Seu problema foi resolvido?",
            style=discord.TextStyle.short,
            required=True,
            max_length=3
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if rating < 1 or rating > 5:
                await interaction.followup.send("❌ Nota deve ser entre 1 e 5.", ephemeral=True)
                return
        except ValueError:
            await interaction.followup.send("❌ Nota inválida.", ephemeral=True)
            return
        
        resolved = self.resolved.value.lower() in ["sim", "s", "yes", "y"]
        
        await self.cog._api_post(
            f"/guilds/{interaction.guild.id}/tickets/{self.ticket_id}/feedback",
            {
        ...
        await interaction.followup.send("✅ Obrigado pelo feedback! Seu comentário nos ajuda a melhorar.", ephemeral=True)
        
        # Notifica no canal de feedback
        feedback_channel_id = await self.cog._get_feedback_log_channel(interaction.guild.id)
        if feedback_channel_id:
            feedback_channel = interaction.guild.get_channel(feedback_channel_id)
            if feedback_channel:
                embed = discord.Embed(
                    title="⭐ Novo Feedback",
                    description=f"Ticket `{self.ticket_id}` recebeu feedback de {interaction.user.mention}",
                    color=discord.Color.gold(),
                    timestamp=discord.utils.utcnow()
                )
                embed.add_field(name="Nota", value=f"{'⭐' * rating} ({rating}/5)", inline=True)
                embed.add_field(name="Resolvido", value="✅ Sim" if resolved else "❌ Não", inline=True)
